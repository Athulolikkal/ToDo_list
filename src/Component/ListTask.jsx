...
const ListTask = ({task,removeTask,taskCompleted}) => {
  return (
   ...
   <input className='checkbox' type="checkbox" name=''  style={{float:'left',cursor:'pointer'}} onChange={(e)=>{taskCompleted(e.target.checked,task.id)}} />
       ...
       <i className="fa fa-trash delete-icon" style={{cursor:'pointer'}}  onClick={()=>{removeTask(task.id)}} ></i>
